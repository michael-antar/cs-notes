...
A _virtual machine_ that runs Java programs by translating platform-independent Java bytecode into machine-specific code.
...
**Concurrency**: Shared by _all threads_

- Because it is shared, it is not thread-safe by default and requires sycnhronization (locks) if multiple threads modify the same object
